    ...
    return (
        <Card className="w-full max-w-md mx-auto bg-card/80 backdrop-blur-sm">
            <CardHeader className="text-center space-y-2">
                <img src="https://storage.googleapis.com/sample-bucket-for-nova/website-images/nova.png" alt="Nova Logo" className="w-16 h-16 mx-auto"/>
            ...
            <CardContent className="grid gap-6">
                 <div className='flex justify-center'>
                     {isLoading ? (
                        <div className='flex justify-center items-center h-10'> <Loader2 className="w-8 h-8 animate-spin text-primary" /> </div>
                    ) : (
                        <GoogleLogin onSuccess={handleLoginSuccess} onError={() => toast.error('Google sign-in failed.')} theme="outline" shape='pill' width="340px"/>
                    )}
                </div>
                <div className="relative">
                    <div className="absolute inset-0 flex items-center"><span className="w-full border-t" /></div>
                    <div className="relative flex justify-center text-xs uppercase">
                        <span className="bg-card px-2 text-muted-foreground">Or a new member?</span>
                    </div>
                </div>
                <Link to="/register">
                    <Button variant="secondary" className="w-full">Create a new account</Button>
                </Link>
            </CardContent>
        </Card>
    );
...
const WelcomePage = ({ showLogin }: { showLogin: boolean }) => {
    return (
        // This container creates the blue gradient background
        <div className="w-full min-h-screen bg-gradient-to-br from-blue-100 via-white to-blue-100 dark:from-gray-900 dark:via-blue-900/20 dark:to-gray-900">
            <header className="absolute top-0 left-0 p-6 flex items-center gap-3">
                <img src="https://storage.googleapis.com/sample-bucket-for-nova/website-images/nova.png" alt="Nova Logo" className="h-8 w-8"/>
                <h1 className='text-xl font-bold tracking-tight'>PROJECT NOVA</h1>
            </header>

            <div className="w-full min-h-screen grid grid-cols-1 lg:grid-cols-2 items-center">
                {/* Left Side: Landing Page Content */}
                <div className="hidden lg:flex flex-col justify-center p-12 text-center lg:text-left">
                    ...
                    <p className="mt-6 max-w-xl text-lg text-muted-foreground">
                        Nova is a collaborative platform for innovators at Google and Cognizant to showcase projects, share technical expertise, and drive the future of cloud technology forward.
                    </p>
                </div>
